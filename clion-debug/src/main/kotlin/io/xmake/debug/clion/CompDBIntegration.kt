 ...
 *
 * CLion-only; lives in the optional `xmake-idea.clion-debug` module so the plugin still loads on
 * IDEA Community (which has neither CIDR nor the compdb plugin).
 */
object CompDBIntegration {
     ...
     * settings and fires listeners, which must not happen on an arbitrary background thread.
     */
    @JvmStatic
